...
out_dir=/data/xfding/train_result/cmlr
data_dir=/data/xfding/data_prep/tts/cmlr_24000/all
...
train_config=conf/tuning/train_pytorch_transformer.small.yaml
decode_config=conf/decode.yaml
#pretrained_model_path=/data/xfding/train_result/csmsc/exp/train_no_dev_pytorch_train_pytorch_transformer.v1.single/results/model.last1.avg.best
# decoding related
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    mkdir -p $out_dir/exp/trim_silence/figs  # avoid error
    echo "Trim silence for"
    ...
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta false \
        $out_dir/data/${eval_set}/feats.scp $out_dir/data/${train_set}/cmvn.ark $out_dir/exp/dump_feats/eval ${feat_ev_dir}
fi
...
#    | sort | uniq | grep -v -e '^\s*$' | awk '{print $0 " " NR+1}' >> ${dict}
    ...
    data2json.sh --feat ${feat_ev_dir}/feats.scp --trans_type phn \
         $out_dir/data/${eval_set} ${dict} > ${feat_ev_dir}/data.json
fi


#train_config="$(change_yaml.py -a pretrained-model="${pretrained_model_path}" \
#    -o "conf/$(basename "${train_config}" .yaml).train_pytorch_transformer.v1.single.yaml" "${train_config}")"
